...
|---|---|---|---|---|---|
| **NOTIFY-01** | A person is actually told when the company needs one | **Blocked** — discoverability **done 2026-09-03** (env example, unit file, runbook "Being told", README, `--help`, startup warning; 3 tests). **Owner's rule (2026-09-03):** use the existing primary human-monitored operations/incident channel; do not create a MyOrg-specific mechanism; credentials stay outside the repo; close only by one notice arriving through the real path. **Inventory of this host, same day:** no notification environment (`MYORG_*`, Slack, webhook, SMTP, pager, Teams — none set); the only authenticated sender on the host is `gh` (GitHub CLI, account DipurajThapa); no mail or chat CLI; Prometheus rules exist in `deploy/` but nothing scrapes or routes them; `company/connectors.md` lists PagerDuty/Datadog/Slack only as *planned, unauthorized* department connectors; the runbook names an "on-call human operator" and no channel. Nothing in the repository or the environment designates a primary incident destination, so none was chosen | **P0** | — | **Human decision, exactly:** name the channel that is already the primary incident destination for MyOrg's operator(s), and the approved way to post to it from the scheduler host. The one candidate that exists today is GitHub (via `gh`) — an issue or comment on this repository — and it is *not* adopted here because nothing establishes it as the monitored incident channel; if it is, say so and it becomes the command. Otherwise supply the existing webhook/CLI |
