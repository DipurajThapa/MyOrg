...
|---|---|---|---|---|---|---|---|---|---|
| VAL-02 acceptance grading | Passes when the grader answers | Grader reachable and returns a parseable `VERDICT:` | Any grader exception → step accepted **unscored**, run completes normally | A deliberate "a grader outage must not strand a worker" choice made the control optional rather than blocking | **None** — one log line, no state, no metric, no notice | None; the run is already `completed` | Probe P4 **[RV]**; `executor.py:211`, `agent_api.graded_failure` | No test asserts what happens when grading fails | **VAL-07** |
| REC-02 leases / step ownership | Granted and honoured by `agent_api` only | Single driver, or all work through the HTTP API | Any in-process `advance()`/`sweep()` alongside an external worker → both do the step | Ownership is a lease *record*, not a precondition of the `complete`/`fail` mutations; the REC-08 fix (drive `in_progress` steps) made the driver claim-blind by design | Only the loser's 409 at submit time, after the spend | Manual; the discarded output is not stored | Probe P5 **[RV]** | No test runs two drivers against one run | **REC-10** |
