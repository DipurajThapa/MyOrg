...
| CI | Catch regressions | compileall, suite, CodeQL, SBOM, npm audit, release gate | ✅ | Green on every push since 2026-09-01; `SUITE: PASS` on ubuntu-24.04, plus a `windows-latest` leg. The suite's `python3` is shimmed to the pinned interpreter — on the dev host that name resolves to 3.13 while `python` is 3.12, so which version ran was previously a matter of PATH | Linux and Windows only; no macOS | — | "Done" is now enforced by a gate, not recorded in a document |
...
| TEST-05 | Cross-platform CI | ✅ | ubuntu-24.04 **and** a `windows-latest` leg running the acceptance suite — the platform this is developed on, and the one carrying `filelock.py`'s `msvcrt` branch | Only the suite runs on Windows; release evidence and the SBOM stay on Linux because they describe the artifact, not the host | — | ~~P1~~ done | `ci.yml` |
...
| VCS-03 | A git remote, so CI actually runs | ✅ | CI had never executed | ~~P1~~ done | Pushed 2026-09-01. **CI ran and passed for the first time**: 3 jobs green (`runtime-and-governance`, `control-center`, `codeql`), and `SUITE: PASS` on **ubuntu-24.04** — the first time the acceptance suite has run on anything but this Windows host, unchanged |
