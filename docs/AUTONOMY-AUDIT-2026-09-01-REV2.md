...
| CI | Catch regressions | compileall, suite, CodeQL, SBOM, npm audit, release gate | ✅ | Green on every push since 2026-09-01: `runtime-and-governance`, `control-center`, `codeql`; `SUITE: PASS` on ubuntu-24.04 | ubuntu-only matrix — Windows, the host it is developed on, is still untested in CI | TEST-05 | "Done" is now enforced by a gate, not recorded in a document |
...
| TEST-05 | Cross-platform CI | 🟡 | ubuntu-24.04, now **executing and green** | A `windows-latest` leg. The irony is sharp: this is developed on Windows and tested only on Linux, so the platform every local run uses is the one CI never checks — and `filelock.py` has a whole Windows branch (`msvcrt`) riding on it | — | P1 | `ci.yml`; run 33552462542 |
